...
class ArrayState {
      ...
      OrderedAwayFrom(this.data[..], next)
  {
    ...
    while i < this.data.Length
      invariant 0 <= i <= this.data.Length
      invariant forall j :: 0 <= j < i ==> this.data[j] == 0
...
input_program.dfy(51,15): Error: assignment might update an array element not in the enclosing context's modifies clause
   |
51 |       this.data[i] := 0;
   |                ^
